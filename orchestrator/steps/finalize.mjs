...
const NEXT_PIPELINE = { dev: 'review', assets: 'review', spec: 'triage', triage: 'dev', bootstrap: 'spec', concept: 'bootstrap', promote: 'qa', qa: 'triage' };
...
// --- promote : la demande est deja partie, il n y a pas de ticket a annoter.
// Le handler existe pour que le chainage vers la QA parte quand meme.
async function finalizePromote({ report }) {
...
export async function finalize(context) {
    ...
    if (report.status !== 'SUCCESS') {
        ...
        const handlers = { dev: finalizePullRequest, assets: finalizePullRequest, review: finalizeReview, spec: finalizeSpec, triage: finalizeTriage, concept: finalizeConcept, qa: finalizeQa, promote: finalizePromote };
