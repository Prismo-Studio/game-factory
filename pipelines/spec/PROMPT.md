...
3. **Découper par travail livrable en une PR**, avec un domaine évident chacun : `gameplay` (la boucle, les entités, les niveaux), `ui` (HUD et écrans propres au jeu), `meta` (progression, upgrades, économie, sauvegarde), `monetisation` (placements, remove ads si spécifique), `art` (un ticket **par asset**, avec dimensions, pivot, points d'attache, slots de palette, usage).
4. **Ordonner par dépendances** : le gameplay minimal (jouable, testable avec des placeholders) d'abord ; l'art en parallèle ; la méta et la monétisation après le gameplay ; le HUD quand les données qu'il affiche existent. Une dépendance est déclarée par index dans `dependsOn`. Pas de cycle.
5. **Rédiger chaque ticket autonome** : quelqu'un qui ouvre uniquement ce ticket, sans lire le GDD, doit comprendre quoi faire et pourquoi. Jamais « voir le GDD » : reprends le contexte utile. Chaque ticket a ses critères d'acceptation, vérifiables, propres à ce ticket. Un ticket art suit le gabarit de `charte/05-contrat-asset.md`.
6. **Rapport** dans ton dernier message.
