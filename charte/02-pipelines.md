...
| 7 | Promote | script | jeu | `develop` en avance sur `main` (battement de cœur, ou réveil par l'auto-merge) | PR de promotion avec `auto-merge` ; c'est `build.yml` du dépôt du jeu qui publie ensuite la release `build-<n>` au push sur `main` | PR de promotion |
